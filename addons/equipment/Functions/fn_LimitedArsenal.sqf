/*
Function: TFD_fnc_LimitedArsenal
...
Description:
 ...
 TFD_fnc_Arsenal, only the items in the container change.
...
Examples:
 (begin example)
  [this] call TFD_fnc_LimitedArsenal;
  [this, "Aegis", 50] call TFD_fnc_LimitedArsenal;
  [this, "VANILLA"] call TFD_fnc_LimitedArsenal;
  [MyAmmoCrate, "RHS", 0] call TFD_fnc_LimitedArsenal;
 (end)
...
call {
 if (_Type isEqualto 0) exitwith {_Type = "TFD"};
...
call {
 if (_Type isEqualTo "TFD") exitwith {
  _AvailableItems =[];
        _AvailableHeadgear = [];
        _AvailableGoggles = [];
        _AvailableUniforms = [];
