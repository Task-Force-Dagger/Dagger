...
Examples:
    (begin example)
        [this, "Reset Targets", [1200,4200,0], 250] call tfd_fnc_ResetTargets;
        [MyLaptopName, "Reset 300m Range", [5500,3522,0], 300] call tfd_fnc_ResetTargets;
    (end)
...
[_center,_distance] call tfd_fnc_SetTargets; // Initial Reset.
...
[
    ...
    "_caller distance _target < 3", {[format["<t color='#FFBB00' size = '.5'>Resetting Targets</t>"],-1,0.8,2,0.5,0,789] spawn BIS_fnc_dynamicText;}, {}, {
        [(_this select 3) select 2, (_this select 3) select 3] call tfd_fnc_SetTargets;
