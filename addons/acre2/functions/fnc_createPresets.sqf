...
private _calcChanFreq = {
    ...
    // e.g. [0,1,1] call tfd_acre2_calcChanFreq
...
tfd_acre2_radioBasenameToSettingsIdx = {
    params ["_radio"];

    private _fndIdx = GVAR(radioCoreSettings) findIf {_radio in (_x select 0)};

    if (_fndIdx isEqualTo -1) then { 
        systemChat format["Warning - TFD ACRE2 Assign - GVAR(radioCoreSettings) missing radio: %1",_radio];
    };
    _fndIdx
};
...
tfd_acre2_networksCreated = true;