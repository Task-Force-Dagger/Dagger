...
/* ----------------------------------------------------------------------------
Function: TFD_acre2_fnc_testNets

Description:
    Checks if players have no radio nets assigned to them.
    Used with TFD Autotest.
...
if (getmissionconfigvalue ["TFD_AcreNetworkEnabled",false]) then {
    LOG("Running Autotest on Radio Networks");
    private _networksArray = getMissionConfigValue ['TFD_AcreSettings', []];
    ...
    UNIQUE(_playerGroups);
 {
        ...
        if (_index == -1) then {
            ...
            private _groupCond = _grp getVariable ["TFD_Network", -1];
