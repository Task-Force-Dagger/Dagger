...
[{
    ...
    if (getMissionConfigValue ['TFD_AcreBabelEnabled',false]) then {
        ...
        } forEach TFD_BabelArray;

        //Check group/unit conditions
        private _groupCond = _unitGroup getVariable ["TFD_BabelLanguages", []];
    ...
    if (!getMissionConfigValue ['TFD_AcreNetworkEnabled',false]) exitWith {
        if (getMissionConfigValue ['TFD_AcreBabelEnabled',false]) then {
    ...
    private _unitVarChannelList = _unit getVariable ["TFD_Channellist", []];
    if (_unitVarChannelList isEqualType "") then { _unitVarChannelList = call compile _unitVarChannelList; };
    private _groupVarChannelList = _unitGroup getVariable ["TFD_Channellist", []];
    ...
    private _isGroupLeader = (leader _unitGroup == _unit);
    {
        ...
        if (_passes) exitWith {
...

            {
                ...
                (_channelsProcessed select ([_radio] call tfd_acre2_radioBasenameToSettingsIdx)) pushBack (_x + [_unitOnThisChannel]);
            } forEach ((_radioPresetSetting select 1)); // loop through channels for the preset
        };
    } forEach GVAR(networksWithRadioChannels);
    if (_ourPresetIndex == -1) then { systemChat "[Warning] TFD-ACRE no preset assigned for myself"; };
    ...
    private _side = _unit getVariable ["tfd_assignGear_side",-1];
    private _faction = _unit getVariable ["tfd_assignGear_faction","civ_f"];
    private _role =  _unit getVariable ["tfd_assignGear_role","baseMan"];
   ...
   //  test = [_side, player getVariable ["tfd_assignGear_faction","civ_f"], player getVariable ["tfd_assignGear_role","baseMan"], "radios"]  call tfd_common_fnc_getCfgEntry;
    //test = [_side, player getVariable ["tfd_assignGear_faction","civ_f"], player getVariable ["tfd_assignGear_role","baseMan"], "radios"]  call tfd_common_fnc_getCfgEntry;
            ...
            // Allocate new radios.
            {
                if (_unit canAdd _x) then {
                    ...
                    if (!_added) then {
                        [_x] spawn {
                            waitUntil{time>3};
                            systemChat format["[TFD ACRE2] Warning: No room to add radio '%1', report this to the mission maker. You now have an addaction to add the radio.",_this select 0];
                        };
                        ...
                        private _actionID = _unit addAction [format ["<t color='#3375D6'>[Radios] Give myself a %1 radio</t>",_radioName],
                        {
                            ...
                            if (_unit canAdd _radioToGive) then {
                                ...
                                systemChat format["[TFD ACRE2] Warning: No room to add radio '%1', remove more stuff and try again",_radioToGive];
                            };
                        }
