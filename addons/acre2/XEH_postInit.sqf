...
/// Parse Radios
private _tfdNetworkEnabled = (getMissionConfigValue ['TFD_AcreNetworkEnabled',false]);
if (_tfdNetworkEnabled) then {
    if (isNil QGVAR(radioCoreSettings)) then {
        GVAR(radioCoreSettings) = [
            ...
            [["ACRE_PRC148"],60,119,0.00625,1,"default"],
            [["ACRE_PRC152"],120,179,0.00625,1,"default"],
            [["ACRE_PRC117F"],180,239,0.00625,1,"default"],
    ...
    GVAR(radioAddActions) = getMissionConfigValue ['TFD_AcreAddRadioActions', []];
    if (GVAR(radioAddActions) isEqualType "") then { GVAR(radioAddActions) = call compile GVAR(radioAddActions)};
    GVAR(networksWithRadioChannels) = getMissionConfigValue ['TFD_AcreSettings', []];
...
if (hasInterface) then {
    [{
        ...
        params ["_tfdNetworkEnabled"];
        if (_tfdNetworkEnabled && isNil "tfd_acre2_networksCreated") exitWith {}; //Ensure presets are created
        if (isNil QEGVAR(common,VarSync)) exitWith {}; // Ensure vars are recieved.

        [] call FUNC(clientInit);
        [_this select 1] call CBA_fnc_removePerFrameHandler;
    }, 1,_tfdNetworkEnabled] call CBA_fnc_addPerFrameHandler;
