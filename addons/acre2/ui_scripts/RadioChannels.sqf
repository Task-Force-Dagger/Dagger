...
switch _mode do {
    ...
    case "refreshChannelTree": {
        ...
        fn_channelTreeProcessUnit = {
            ...
            if (_giveRadio) then {
                _ctrlTree tvSetPictureRight [_location, "z\tfd\addons\acre2\ui\RadioIcon_Radio_Small.paa"];
                _returnCode = 0;
            } else {
                _ctrlTree tvSetPictureRight [_location, "z\tfd\addons\acre2\ui\RadioIcon_None_Small.paa"];
            };
        ...
        fn_channelTreeProcessGroup = {
            ...
            if (_giveRadio) then {
                ...
                if (_giveLeader) then {
                    _ctrlTree tvSetPictureRight [_location, "z\tfd\addons\acre2\ui\RadioIcon_Leader_Small.paa"];
                    _returnCode = 2;
                } else {
                    if (_gaveSomeoneARadio) then {
                        _ctrlTree tvSetPictureRight [_location, "z\tfd\addons\acre2\ui\RadioIcon_Partial_Small.paa"];
        ...
        private _sides = []; {
            ...
            if (_x isEqualType east) then {    
                ...
                if (_sides pushBackUnique _side != -1) then {
                    ...
                    _ctrlTree tvSetPicture [_location, _side call TFD_common_fnc_sideToTexture];
    ...
    case "refreshNetworkTree": {
        ...
        _factions = []; {
            ...
            //Find Side
            if (_sideIdx == -1) then {
                private _sideText = _side call TFD_common_fnc_sideToString; 
                private _sideIcon = _side call TFD_common_fnc_sideToTexture; 
                ...
                if (_networkNumber != -1) then {
                    _ctrlTree tvSetPictureRight [[_sideIdx], (_networkNumber+1) call TFD_common_fnc_numToTexture];
                } else {
            ...
            if (_factionIdx == -1) then {
                ...
                if (_networkNumber != -1) then {
                    _ctrlTree tvSetPictureRight [[_sideIdx, _factionIdx], (_networkNumber+1) call TFD_common_fnc_numToTexture];
                } else {
