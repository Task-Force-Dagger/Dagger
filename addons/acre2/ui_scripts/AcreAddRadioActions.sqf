...
switch _mode do {
    case "onLoad": {
        ...
        _ctrlList lnbSetColumnsPos [0.1,0.2];
        {
            ...
            _alpha = if (_x in TFD_AcreAddRadioActions_Array) then {1} else {0.5};
            _ctrlList lnbsetcolor [[_lnbAdd,1],[1,1,1,_alpha]];
        } forEach ACRE_RADIO_CLASSNAME_ARRAY;
    };
    case "attributeSave": {
        str (uiNamespace getVariable "TFD_AcreAddRadioActions_Array")
    };
    case "listModify": {
        ...
        if (_radio in TFD_AcreAddRadioActions_Array) then {
            TFD_AcreAddRadioActions_Array = TFD_AcreAddRadioActions_Array - [_radio];
        } else {
