 ...
 * \TFD_common\functions\sounds\fn_getSoundVolume.sqf
 * by Ojemineh
 *
 ...
 *
 * Example:
 * ["AlarmBell"] call TFD_common_fnc_getSoundVolume;
 *
 */
...
if (count _soundArray > 1) then {
    if ((_soundArray select 1) isEqualType "") then {
        if (([(_soundArray select 1), "db"] call CBA_fnc_find) > -1) then {
            _return = [(_soundArray select 1)] call TFD_common_fnc_decibelToGain;
        } else {
