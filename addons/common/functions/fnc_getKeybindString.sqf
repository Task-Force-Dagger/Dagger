 ...
 * \TFD_common\functions\settings\fn_getKeybindString.sqf
 * by Ojemineh
 * 
 * get keybind string
 * 
 ...
 * 
 * Example:
 * ["CH View Distance", "open_settings"] call TFD_common_fnc_getKeybindString;
 * 
 */
...
if (!isNil "_entry") then {
    ...
    private _modifier = 
        (if (_useShift) then {localize "STR_TFD_KeyBinds_Shift_Modifier"} else {""}) +
        (if (_useCtrl) then {localize "STR_TFD_KeyBinds_Ctrl_Modifier"} else {""}) +
        (if (_useAlt) then {localize "STR_TFD_KeyBinds_Alt_Modifier"} else {""});
