 ...
 * \TFD_common\functions\sounds\fn_playSound3d.sqf
 * by Ojemineh
 *
 ...
 *
 * Example:
 * ["AlarmBell", player, (player modelToWorld [0,0,3])] call TFD_common_fnc_playSound3d;
 *
 */
    ...
    if (_volume > 0) then {
        ...
        if ((_soundArray select 1) isEqualType "") then {
            if (([(_soundArray select 1), "db"] call CBA_fnc_find) > -1) then {
                _soundVolume = [(_soundArray select 1)] call TFD_common_fnc_decibelToGain;
            } else {
