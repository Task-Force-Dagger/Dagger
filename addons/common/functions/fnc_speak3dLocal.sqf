 ...
 * \TFD_common\functions\sounds\fn_speak3dLocal.sqf
 * by Ojemineh
 *
 ...
 *
 * Example:
 * [player, "cp_mission_accomplished_1", -1, -1, 4.5] call TFD_common_fnc_speak3dLocal;
 *
 */
...
if (_distance <= 0) then {_distance = [_soundClass] call TFD_common_fnc_getSoundDistance;};
if (_distance <= 0) then {_distance = 30;};
if ((_maxDistance > 0) && ((player distance _unit) > _maxDistance)) exitWith {};

if (_duration <= 0) then {_duration = [_soundClass] call TFD_common_fnc_getSoundDuration;};
if (_duration <= 0) exitWith {};

if (_pitch <= 0) then {_pitch = [_soundClass] call TFD_common_fnc_getSoundPitch;};
if (_pitch <= 0) then {_pitch = 1;};

// -------------------------------------------------------------------------------------------------

private _soundArray = _unit getVariable ["TFD_sound3d", []];
...
if (_soundInUse) exitWith {
    [_unit, _soundClass, _distance, _maxDistance, _duration, _pitch, _isSpeech] spawn {
        ...
        [_unit, _soundClass] call TFD_common_fnc_stop3dSoundLocal;
        sleep 0.1;
        [_unit, _soundClass, _distance, _maxDistance, _duration, _pitch, _isSpeech] call TFD_common_fnc_speak3dLocal;
    };
};
...
[_unit, _soundClass, _distance, _maxDistance, _duration, _pitch, _isSpeech] spawn {
    ...
    private _helper = createSimpleObject [TFD_COMMON_HELPER_OBJECT, [0,0,0], true];
    ...
    _unit setVariable ["TFD_sound3d", _soundArray];
    _unit setVariable ["TFD_speak3d", true, true];
    ...
    _unit setVariable ["TFD_speak3d", nil, true];
