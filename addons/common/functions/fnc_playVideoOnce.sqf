 ...
 * \TFD_common\functions\videos\fn_playVideoOnce.sqf
 * by Ojemineh
 * 
 ...
 * 
 * Example:
 * ["intro.ogv"] call TFD_common_fnc_playVideoOnce;
 * 
 */
...
private _return = [_filename, _videoID, _canSkip, _showSkip, _showTime] spawn {
    ...
    private _videoTag = format ["TFD_Video_%1", _videoID];
    ...
    if ( (_canSkip) && (_showSkip) ) then {
        _script = [_showTime] spawn {
            ...
            private _skipMsg = format ["<t font='RobotoCondensed' color='#ffffff' shadow='2' size='1.6'>%1</t>", localize "STR_TFD_Message_SkipVideo"];
    ...
    private _keyHandler = (findDisplay 46) displayAddEventHandler ["KeyDown", "_this call TFD_common_fnc_videoKeyDown"];
