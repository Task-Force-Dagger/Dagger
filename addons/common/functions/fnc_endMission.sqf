...
/*
 * Name: TFD_common_fnc_endMission
 * Author: Snippers
 *
 * Arguments:
...
// Message admins
private _message = format["[TFD] Ending mission..."];
...
if (!isNil "ocap_fnc_exportData") then {
    _message = format["[TFD] OCAP detected exporting (10 second timeout)..."];
    [_message,'tac1_admin_fnc_messageAdmin',true] call BIS_fnc_MP;
    ...
    [] spawn {
         {
            ...
            private _message = format["[TFD] OCAP Export complete. Ending..."];
            [_message,'tac1_admin_fnc_messageAdmin',true] call BIS_fnc_MP;
            GVAR(endMissionWait) = -1;
