...
/*
 * Name = TFD_assignGear_fnc_chat_loadout
 * Author = Freddo
 *
 * Command syntaxes:
...
switch (count _inputArr) do {
    ...
    case 1: {
        ...
        if (isClass (_cfg >> "CfgLoadouts" >> _faction >> _in1)) then {
            ...
            systemChat format ["TFD: Assigned loadout %1", str getText (_cfg >> "CfgLoadouts" >> _faction >> _in1 >> "displayName")];
        } else {
            private _match = [_in1] call FUNC(findMatch);
            if (isNull _match) then {
                // No loadout or player found, or more than one player
                if (_faction isEqualTo "") then {
                    systemChat "TFD Error: Cannot select loadout as you do not have an assigned faction. Use #loadout <faction> <role>";
                    systemChat FORMAT_1("TFD Error: Could not find player containing %1, or more than one player found.", str _in1);
                } else {
                    systemChat FORMAT_2("TFD Error: No loadout with name %1 in %2", str _in1, _faction);
                    systemChat FORMAT_1("TFD Error: Could not find player containing %1, or more than one player found.", str _in1);
                };
            } else {
                // Copy other players loadout
                CURUNIT setUnitLoadout getUnitLoadout _match;
                systemChat FORMAT_1("TFD: Copied loadout from %1", name _match);
            };
        };
    };
    case 2: {
        ...
        if (isClass (_cfg >> "CfgLoadouts" >> _in1 >> _in2)) then {
            ...
            systemChat FORMAT_2("TFD: Assigned loadout %1 from %2", \
            ...
            if !(isClass (_cfg >> "CfgLoadouts" >> _in1)) then {
                systemChat FORMAT_1("TFD Error: No faction with name %1", str _in1);
            } else {
                systemChat FORMAT_2("TFD Error: No loadout with name %1 in %2", str _in2, str _in1);
            };
        };
    };
};
