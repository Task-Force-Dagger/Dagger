...
if (_name isEqualTo "") exitWith {
    systemChat "TFD Error: No argument passed. Use #tp <player> or <group>";
};
...
if (!isNull _unit) then {
    if (vehicle _unit != _unit) then {
        ...
        if !(CURUNIT in vehicle _unit) then {
            systemChat FORMAT_1("TFD: %1's vehicle is full", name _unit);
        } else {
            systemChat FORMAT_1("TFD: Teleported into %1's vehicle", name _unit);
        };
    } else {
        private _pos = _unit getRelPos [3, 180];
        CURUNIT setDir (getDir _unit);
        CURUNIT setPos _pos;
        systemChat FORMAT_1("TFD: Teleported to %1", name _unit);
    };
} else {
    systemChat FORMAT_1("TFD Error: No player/group containing %1, or more than one found.", str _name);
};
