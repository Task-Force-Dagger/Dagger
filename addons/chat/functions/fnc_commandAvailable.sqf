...
/*
 * Name = TFD_chat_fnc_commandAvailable
 * Author = Freddo
 *
 * Parameters:
...
switch (_var) do {
    case 0: { // Never available
        if true exitWith {
            systemChat FORMAT_1("TFD: %1 is disabled.", _command);
            _enabled = false;
        };
    };
    case 1: { // Available during safestart
        if !(call EFUNC(safestart,isActive)) exitWith {
            systemChat FORMAT_1("TFD: %1 is only available during Safe Start.", _command);
            _enabled = false;
        };
    };
