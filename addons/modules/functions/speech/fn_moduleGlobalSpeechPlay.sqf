 ...
 * \z\tfd\addons\modules\functions\speech\fn_moduleGlobalSpeechPlay.sqf
 * by Ojemineh
 * 
 * module function
 * 
 ...
 * 
 * Example:
 * [] call TFD_fnc_moduleGlobalSpeechPlay;
 * 
 */
...
[_logic, _caller, _target, _source, _sound, _distance, _maxDistance, _duration, _onlyOnce, _reaction, _animation, _codeStart, _codeEnd] spawn {
    ...
    if (_onlyOnce) then {_target setVariable ["tfd_playedOnce", true, true];};
    ...
    [_target, _sound, _distance, _maxDistance, _duration, 1, true] call TFD_fnc_speak3d;
    ...
    waitUntil {if (!(_target getVariable ["tfd_speak3d", false])) exitWith {true}; false};
