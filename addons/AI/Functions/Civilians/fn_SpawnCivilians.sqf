/*
Function: tfd_fnc_SpawnCivilians
...
if !(_pedPool isEqualTo []) then {
   private _pedDif = _pedMax - _pedMin;
    ...
    for "_x" from 1 to (_pedMin + floor(random _pedDif)) do {
        ...
        // Setup patrol task
        [_g, _roadList] call tfd_fnc_CivilianPatrol;
        _units append (units _g);
    };
} else {
    if (_AIReporting && { _pedMax > 0 }) exitwith {systemchat format ["TFD: INFO: %1 no Civilian peds to select from. Step skipped.",_faction]};
};
...
if !((_motPool isEqualTo []) || (_pedPool isEqualTo [])) then {
   private _vehpatrolDif = _vehpatrolMax - _vehpatrolMin;
    ...
    if (_AIReporting && { _vehpatrolMax > 0 }) exitwith {systemchat format ["TFD: INFO: %1 no Civilian peds or vehicles to select from. Step skipped.",_faction]};
};
...
if !(_motPool isEqualTo []) then {
   private _vehparkedDif = _vehparkedMax - _vehparkedMin;
    ...
    if (_AIReporting && { _vehparkedMax > 0 }) exitwith {systemchat format ["TFD: INFO: %1 no Civilian vehicles to select from. Step skipped.",_faction]};
};
