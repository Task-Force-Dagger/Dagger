...
_garrisons params ["_GarrisonedGroupsMin", ["_GarrisonedGroupsMax", 0], ["_infSkill", "TFD Default"]];
_inf params ["_infMin", ["_infMax", 0], ["_infSkill", "TFD Default"]];
_infaa params ["_infaaMin", ["_infaaMax",0], ["_infaaSkill", "TFD Default"]];
_infat params ["_infatMin", ["_infatMax",0], ["_infatSkill", "TFD Default"]];
_sniper params ["_sniperMin", ["_sniperMax",0], ["_sniperSkill", "TFD Default"]];
_vehaa params ["_vehaaMin", ["_vehaaMax",0], ["_vehaaSkill", "TFD Default"]];
_vehmrap params ["_vehmrapMin", ["_vehmrapMax",0], ["_vehmrapSkill", "TFD Default"]];
_vehlight params ["_vehlightMin", ["_vehlightMax",0], ["_vehlightSkill", "TFD Default"]];
_vehheavy params ["_vehheavyMin", ["_vehheavyMax",0], ["_vehheavySkill", "TFD Default"]];
_vehrand params ["_vehrandMin", ["_vehrandMax",0], ["_vehrandSkill", "TFD Default"]];
...
switch (_patrolMethod) do {
    case "ROAD": {
        _fnc_patrol_EI = compile "[(_this select 0), (_this select 3)] call tfd_fnc_RoadPatrol;";
        _fnc_patrol_EI_spec = compile "[(_this select 0), (_this select 3)] call tfd_fnc_RoadPatrol;";
        _fnc_patrol_veh = compile "[(_this select 0), (_this select 3)] call tfd_fnc_RoadPatrol;";
    ...
    case "RANDOM": {
        ...
        _fnc_pos_EI = compile "[[[(_this select 0), (_this select 1)],[]],[""water""]] call tfd_fnc_SafePos;";
        _fnc_pos_veh = compile "[[[(_this select 0), (_this select 1)], []], [""water""], { !(_this isFlatEmpty [2,-1,0.5,1,0,false,objNull] isEqualTo []) }] call tfd_fnc_SafePos;";
    };
    default {
        _fnc_patrol_EI = compile "systemChat ""Error: Unknown patrol method supplied to tfd_fnc_SpawnAI!"";";
        _fnc_patrol_EI_spec = compile "systemChat ""Error: Unknown patrol method supplied to tfd_fnc_SpawnAI!"";";
        _fnc_patrol_veh = compile "systemChat ""Error: Unknown patrol method supplied to tfd_fnc_SpawnAI!"";";
        _fnc_pos_EI = compile "systemChat ""Error: Unknown patrol method supplied to tfd_fnc_SpawnAI!"";";
        _fnc_pos_veh = compile "systemChat ""Error: Unknown patrol method supplied to tfd_fnc_SpawnAI!"";";
    };
};
...
private _AIReporting = TFD_AI_Reporting;
...
_GetFactionArrays = compileFinal format ["call tfd_fnc_%1", _faction];
...
if (_patrolMethod isEqualTo "ROAD") then {
    ...
    if (count _roadList < _minPositions) then {
        while {count _roadList < _minPositions} do {
            _rpos = [[[_center, _radius],[]],["water"]] call tfd_fnc_SafePos;
...
if !(_infList isEqualTo []) then {
   private _InfDif = _infMax - _infMin;
    ...
    for "_x" from 1 to (_infMin + floor(random _InfDif)) do {
        ...
        [_g, _infSkill] call tfd_fnc_SetUnitSkill;
        _units append (units _g);
    };
} else {
    if (_AIReporting && { _infMax >0 }) exitwith {systemchat format ["TFD: INFO: %1 no Infantry Teams to select from. Step skipped.",_FactionName]};
    };
...
if !(_infaaList isEqualTo []) then {
   private _InfaaDif = _InfaaMax - _InfaaMin;
    ...
    for "_x" from 1 to (_InfaaMin + floor(random _InfaaDif)) do {
        ...
        [_g, _infaaSkill] call tfd_fnc_SetUnitSkill;
        _units append (units _g);
    };
} else {
    if (_AIReporting && { _InfaaMax >0 }) exitwith {systemchat format ["TFD: INFO: %1 no AA Teams to select from. Step skipped.",_FactionName]};
    };
...
if !(_infatList isEqualTo []) then {
   private _InfatDif = _InfatMax - _InfatMin;
    ...
    for "_x" from 1 to (_InfatMin + floor(random _InfatDif)) do {
        ...
        [_g, _center, _radius, _waypoints] call _fnc_patrol_EI_spec;
        [_g, _infatSkill] call tfd_fnc_SetUnitSkill;
        _units append (units _g);
    };
} else {
    if (_AIReporting && { _InfatMax >0 }) exitwith {systemchat format ["TFD: INFO: %1 no AT Teams to select from. Step skipped.",_FactionName]};
    };
...
if !(_sniperList isEqualTo []) then {
   private _SniperDif = _SniperMax - _SniperMin;
    ...
    for "_x" from 1 to (_SniperMin + floor(random _SniperDif)) do {
        ...
        [_g, _sniperSkill] call tfd_fnc_SetUnitSkill;
        _units append (units _g);
    };
} else {
    if (_AIReporting && { _SniperMax >0 }) exitwith {systemchat format ["TFD: INFO: %1 no Sniper Teams to select from. Step skipped.",_FactionName]};
    };
   ...
   private _VehAADif = _VehAAMax - _VehAAMin;
    ...
    for "_x" from 1 to (_VehAAMin + floor(random _VehAADif)) do {
        ...
        if !(_rpos isEqualTo [0,0]) then {
            ...
            [_g, _vehaaSkill] call tfd_fnc_SetUnitSkill;
    ...
    if (_AIReporting && { _VehAAMax >0 }) exitwith {systemchat format ["TFD: INFO: %1 no AA Vehicles to select from. Step skipped.",_FactionName]};
    };
   ...
   private _VehMRAPDif = _VehMRAPMax - _VehMRAPMin;
    ...
    for "_x" from 1 to (_VehMRAPMin + floor(random _VehMRAPDif)) do {
        ...
        if !(_rpos isEqualTo [0,0]) then {
            ...
            [_v, _g] call BIS_fnc_spawnCrew;
            [_g, _center, _radius, _waypoints] call _fnc_patrol_veh;
            [_g, _vehmrapSkill] call tfd_fnc_SetUnitSkill;
            if (random 1 >= 0.5) then { _v allowCrewInImmobile true; };
    ...
    if (_AIReporting && { _VehMRAPMax >0 }) exitwith {systemchat format ["TFD: INFO: %1 no MRAPs to select from. Step skipped.",_FactionName]};
    };
   ...
   private _VehLightDif = _VehLightMax - _VehLightMin;
    ...
    for "_x" from 1 to (_VehLightMin + floor(random _VehLightDif)) do {
        ...
        if !(_rpos isEqualTo [0,0]) then {
            ...
            [_v, _g] call BIS_fnc_spawnCrew;
            [_g, _center, _radius, _waypoints] call _fnc_patrol_veh;
            [_g, _vehLightSkill] call tfd_fnc_SetUnitSkill;
            if (random 1 >= 0.5) then { _v allowCrewInImmobile true; };
    ...
    if (_AIReporting && { _VehLightMax >0 }) exitwith {systemchat format ["TFD: INFO: %1 no Light Vehicles to select from. Step skipped.",_FactionName]};
    };
   ...
   private _VehHeavyDif = _VehHeavyMax - _VehHeavyMin;
    ...
    for "_x" from 1 to (_VehHeavyMin + floor(random _VehHeavyDif)) do {
        ...
        if !(_rpos isEqualTo [0,0]) then {
            ...
            [_v, _g] call BIS_fnc_spawnCrew;
            [_g, _center, _radius, _waypoints] call _fnc_patrol_veh;
            [_g, _vehHeavySkill] call tfd_fnc_SetUnitSkill;
            if (random 1 >= 0.5) then { _v allowCrewInImmobile true; };
    ...
    if (_AIReporting && { _VehHeavyMax >0 }) exitwith {systemchat format ["TFD: INFO: %1 no Heavy Vehicles to select from. Step skipped.",_FactionName]};
    };
   ...
   private _VehRandDif = _VehRandMax - _VehRandMin;
    ...
    for "_x" from 1 to (_VehRandMin + floor(random _VehRandDif)) do {
        ...
        if !(_rpos isEqualTo [0,0]) then {
            ...
            [_v, _g] call BIS_fnc_spawnCrew;
            [_g, _center, _radius, _waypoints] call _fnc_patrol_veh;
            [_g, _vehrandSkill] call tfd_fnc_SetUnitSkill;
            if (random 1 >= 0.5) then { _v allowCrewInImmobile true; };
