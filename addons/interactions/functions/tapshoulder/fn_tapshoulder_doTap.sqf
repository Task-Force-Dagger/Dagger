 ...
 * \z\tfd\addons\interactions\functions\tapshoulder\fn_tapshoulder_doTap.sqf
 * by Ojemineh
 * 
 ...
 * 
 * Example:
 * [player, unit1, 0] call TFD_interactions_fnc_tapshoulder_doTap;
 * 
 */
...
[_player, _target, _shoulder] spawn {
    ...
    if (missionNamespace getVariable ["tfd_interactions_global_sounds", false]) then {
        ...
        ["tfd_Action_Shoulder_Tap_1", _target, AGLToASL (_target modelToWorld (_target selectionPosition _selectionName)), 20, 1, _pitch, false] call TFD_common_fnc_playSound3d;
    } else {
        playSound "tfd_Action_Shoulder_Tap_1";
    };
    
    if (isPlayer _target) then {
        [_player, _target, _shoulder] remoteExecCall ["tfd_interactions_fnc_tapshoulder_tap", _target];
    };
