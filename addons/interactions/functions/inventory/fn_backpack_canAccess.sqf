 ...
 * \z\tfd\addons\interactions\functions\inventory\fn_backpack_canAccess.sqf
 * by Ojemineh
 * 
 ...
 * 
 * Example:
 * [player, unit1] call TFD_interactions_fnc_backpack_canAccess;
 * 
 */
...
if ((side _player) == (side _target)) then {
    if ((group _player) == (group _target)) then {
        if (missionNamespace getVariable ["tfd_interactions_backpack_access_faction", 0] >= 1) then {_enabled = true;};
    } else {
        if (missionNamespace getVariable ["tfd_interactions_backpack_access_faction", 0] == 2) then {_enabled = true;};
    };
} else {
    if (_isFriendly) then {
        if (missionNamespace getVariable ["tfd_interactions_backpack_access_friendly", 0] > 0) then {_enabled = true;};
    } else {
        if (missionNamespace getVariable ["tfd_interactions_backpack_access_enemy", 0] > 0) then {_enabled = true;};
    };
};
