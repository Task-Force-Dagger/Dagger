 ...
 * \z\tfd\addons\interactions\functions\inventory\fn_gear_access.sqf
 * by Ojemineh
 * 
 * search targets gear
 * 
 ...
 * 
 * Example:
 * [player, unit1] call TFD_interactions_fnc_gear_access;
 * 
 */
...
if (missionNamespace getVariable ["tfd_interactions_gear_access_hint", true]) then {
    ...
    private _iconFile = "z\tfd\addons\interactions\data\icons\search_gear_ca.paa";
    private _icon = format [_iconTemplate, _iconFile];
    private _hintMessage = format [localize "STR_TFD_Interactions_Hint_Gear_Access", _playerName, _icon];
    ...
    if !(missionNamespace getVariable ["tfd_interactions_global_sounds", false]) then {
        private _soundClass = selectRandom ["tfd_Action_Backpack_Search_1"];
