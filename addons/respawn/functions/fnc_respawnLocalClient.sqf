...
#include "\z\tfd\addons\respawn\script_component.hpp"
...
tfd_localRespawnedUnit = objNull;
/// Create the unit
private _unitName = format["respawnedUnit%1",_number];

private _init = format["if (local this) then { [this, '%2', '%3'] call tfd_assignGear_fnc_assignGear; tfd_localRespawnedUnit = this; }; this setName '%4'; this setVariable ['tfd_isRespawnUnit',true];  %1 = this; ",_unitName, _faction, _typeOfUnit, profileName];
private _oldUnit = player;

_class createUnit [_position, _dummyGroup, _init, 0.5, _rankName];    

// Wait till the unit is created
waitUntil{!isNull tfd_localRespawnedUnit};
...
setPlayable tfd_localRespawnedUnit; // Allow unit to referenced later with player functions (name/getPlayerUID) (Verify this works)
selectPlayer tfd_localRespawnedUnit;
...
if (isClass(configFile >> "CfgPatches" >> "acre_main")) then  {
    ...
    [{
        ...
        if (isNil "tfd_acre2_networksCreated") exitWith {}; //Ensure presets are created
