        ...
        _factions sort true;
 {
    ...
    case "refreshDeadListBox": {
        lbClear _ctrlDeadListBox; {
            ...
            if (!_found) then {
                private _name = _deadPlayer getVariable ["tfd_spectator_name",name _deadPlayer];
    ...
    case "respawnMapLoaded": {
        ...
        private _pos = [0,0,0];
        if (alive player && !(player isKindOf "tfd_spectator_unit")) then {
          _pos = getPos player;  
        } else {
    ...
    case "respawnMap_onMouseButtonDown": {
        ...
        if (_type == 0) then { // left click
            private _i = 1;
            private _found = false;
            private _var = missionNamespace getVariable[format["tfd_respawnPoint%1",_i],objNull];
            while {!(isNull _var)} do {
                ...
                _i = _i + 1;
                _var = missionNamespace getVariable[format["tfd_respawnPoint%1",_i],objNull];
            };
            if (!_found) then {
        ...
        if (_type == 28) then {
            if (GVAR(respawnMousePos) isEqualTo -1) then {
                ...
                if (GVAR(respawnMousePos) isEqualType []) then {
                    ...
                    private _var = missionNamespace getVariable[format["tfd_respawnPoint%1",GVAR(respawnMousePos)],objNull];
                    if (!isNull _var) then {
