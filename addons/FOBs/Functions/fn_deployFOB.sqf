...
if (!isServer) exitWith {
    _this remoteExec ["TFD_fnc_deployFOB", 2];
};

params ["_object", "_caller"];

// sanity checks
if (_object getVariable ["TFD_PortableFOB_Deployed", false]) exitWith {
    systemChat "FOB was already deployed!";
};

_type = _object getVariable ["TFD_PortableFOB_Type", "NULL"];

if (_type isEqualTo "NULL") exitWith {
    systemChat "Invalid FOB type set!";
};

_name = _object getVariable ["TFD_PortableFOB_Name", ""];

_size = _object getVariable ["TFD_PortableFOB_Size", -1];
...
_compArray = call (compile format ["call TFD_fnc_%1", _type]);
...
_object setVariable ["TFD_PortableFOB_Objects", _objects, true];
_object setVariable ["TFD_PortableFOB_HiddenObjects", _hiddenObjects, true];
_object setVariable ["TFD_PortableFOB_Deployed", true, true];
_object setVariable ["TFD_PortableFOB_Pos", _pos, true];
_object setVariable ["TFD_PortableFOB_RespawnPos", _respawnPos, true];
