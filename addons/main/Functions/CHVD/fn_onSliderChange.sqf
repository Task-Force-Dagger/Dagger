...
_viewDistValue = _sliderPos min TFD_Main_ViewDistanceMaxDistance max 0;
_objViewDistValue = if (_modeVar isEqualTo 1) then {_sliderPos  * (call compile _percentVar) min TFD_Main_ViewDistanceMaxObjectDistance} else {_sliderPos min TFD_Main_ViewDistanceMaxObjectDistance};  // Check if percentage sync mode is used, if so use a percentage coefficient
