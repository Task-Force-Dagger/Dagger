...
_viewDistValue = _textValue min TFD_Main_ViewDistanceMaxDistance;
_objViewDistValue = if (_modeVar isEqualTo 1) then {_textValue  * (call compile _percentVar) min TFD_Main_ViewDistanceMaxObjectDistance} else {_textValue min TFD_Main_ViewDistanceMaxObjectDistance};  // Check if percentage sync mode is used, if so use a percentage coefficient
