...
switch (_updateMode) do {
    case 1: {
        _viewDistValue = _viewDist + _updateValue min TFD_Main_ViewDistanceMaxDistance max 500;  
        
        _percentVar = "CHVD_" + _vehTypeString + "SyncPercentage";
        _percentValue = call compile _percentVar;
        
        _objViewDistValue = _viewDistValue * _percentValue min TFD_Main_ViewDistanceMaxObjectDistance;
    ...
    case 2: {  
        _objViewDistValue = _objViewDist + _updateValue min _viewDist min TFD_Main_ViewDistanceMaxObjectDistance max 500;
        call compile format ["%1 = %2", _objViewDistVar, _objViewDistValue];
        call compile format ["profileNamespace setVariable ['%1',%1]", _objViewDistVar];
    ...
    default {
        _viewDistValue = _viewDist + _updateValue min TFD_Main_ViewDistanceMaxDistance max (500 + _vdDiff);
        _objViewDistValue = _objViewDist + _updateValue min (_viewDistValue - _vdDiff) min TFD_Main_ViewDistanceMaxObjectDistance max 500;
        call compile format ["%1 = %2", _viewDistVar, _viewDistValue];
        call compile format ["profileNamespace setVariable ['%1',%1]", _viewDistVar];
...
for "_i" from 1 to (35) step 1 do {
    if ((call compile _viewDistVar) < TFD_Main_ViewDistanceMaxDistance / 35 * _i) then {
        _vdString = _vdString + "·";
    } else { 
...
_ovdString = "";
for "_i" from 1 to (35) step 1 do {
    if ((call compile _objViewDistVar) < TFD_Main_ViewDistanceMaxObjectDistance / 35 * _i) then {
