...
private _binding = ["TFD", QGVAR(openKey)] call CBA_fnc_getKeybind;
...
if (_handleKeypress) then {
    if (_authorized) then {
        if (dialog && !isNull (uiNamespace getVariable [QGVAR(display), displayNull])) then {
            systemChat "[TFD Admin Menu] The admin menu is already open"
        } else {
            if (!isNull (findDisplay 312)) then {
                systemChat "[TFD Admin Menu] Can't open the admin menu in the Zeus interface";
            } else {
    ...
    _binding = ["TFD", QGVAR(spectatorRemoteControl)] call CBA_fnc_getKeybind;
    if (isNil "_binding") exitWith {};
    (_binding select 5) params ["_DIK", "_modifiers"];
        ...
        if (_authorized) then {
            if (isNull (findDisplay 5454)) then {
                if (isNull (missionNamespace getVariable ["bis_fnc_moduleRemoteControl_unit", objNull])) then {
                    systemChat "[TFD Admin Menu] Remote Control is available only through TFD Spectator";
                } else {
                    if (player isKindOf QEGVAR(spectator,unit)) then {
                        systemChat "[TFD Admin Menu] Use scroll menu action to stop remote control";
                    };
                };
            } else {
                if (isNil QEGVAR(spectator,target)) then {
                    systemChat "[TFD Admin Menu] No unit selected for Remote Control.";
                } else {
...
if (_handleKeypress && !_authorized) then {
    systemChat "[TFD Admin Menu] You're not authorized to use the admin menu";
};
