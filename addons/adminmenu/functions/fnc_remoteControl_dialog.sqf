...
if (isNil QGVAR(remoteControlUnits) || {GVAR(remoteControlUnits) isEqualTo []}) exitWith {
    systemChat "[TFD Admin Menu] No unit candidates for remote control";
};
...
_ctrlOK ctrlSetPosition [(_dialogPosition # 0) + 0.67 * (_dialogPosition # 2), (_dialogPosition # 1) + (_dialogPosition # 3) + 0.1 * TFD_ADMINMENU_STD_HEIGHT, 0.33 * (_dialogPosition # 2), TFD_ADMINMENU_STD_HEIGHT];
