...
private _control = (_display displayCtrl IDC_TFD_ADMINMENU_RESP_FACTIONCATEGORY); /* respawnMenuFactionCategoryCombo */
lbClear _control;
...
_control = (_display displayCtrl IDC_TFD_ADMINMENU_RESP_FACTION); /* respawnMenuFactionCombo */
lbClear _control;
...
_control = (_display displayCtrl IDC_TFD_ADMINMENU_RESP_SIDE); /* Side control */
lbClear _control;
...
(_display displayCtrl IDC_TFD_ADMINMENU_RESP_SPECTATORTEXT) ctrlSetText format["Players in Spectator: %1",count GVAR(spectatorList) ];
...
// Group ID
_control = (_display displayCtrl IDC_TFD_ADMINMENU_RESP_GROUPNAME);
...
// Marker Name
_control = (_display displayCtrl IDC_TFD_ADMINMENU_RESP_GROUPMARKERNAME);
...
if (!isNil QGVAR(respawnGroupMarkerCheckBoxVal)) then {
    (_display displayCtrl IDC_TFD_ADMINMENU_RESP_GROUPMARKERCHECKBOX) cbSetChecked GVAR(respawnGroupMarkerCheckBoxVal);
} else {
...
[{
    ...
    _display = findDisplay IDD_TFD_ADMINMENU;
    if (isNull (findDisplay IDD_TFD_ADMINMENU)) exitWith { [_pfhID] call CBA_fnc_removePerFrameHandler;} ;
