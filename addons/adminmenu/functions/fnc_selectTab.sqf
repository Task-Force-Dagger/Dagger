...
params ["_display", ["_tab", IDC_TFD_ADMINMENU_G_DASH]];
...
} forEach IDCS_TFD_ADMINMENU_GRPS;

switch (_tab) do {
    case IDC_TFD_ADMINMENU_G_DASH: {
        LOG("Selected Dashboard");
        ctrlSetFocus (_display displayCtrl IDC_TFD_ADMINMENU_DASH);
        _display call FUNC(dashboard);
    };
    case IDC_TFD_ADMINMENU_G_PMAN: {
        LOG("Selected Player Management");
        ctrlSetFocus (_display displayCtrl IDC_TFD_ADMINMENU_PMAN);
        _display call FUNC(playerManagement);
    };
    case IDC_TFD_ADMINMENU_G_RESP: {
        LOG("Selected Respawn");
        ctrlSetFocus (_display displayCtrl IDC_TFD_ADMINMENU_RESP);
        _display call FUNC(respawn);
    };
    case IDC_TFD_ADMINMENU_G_ENDM: {
        LOG("Selected End Mission");
        ctrlSetFocus (_display displayCtrl IDC_TFD_ADMINMENU_ENDM);
        _display call FUNC(endMission);
    };
    case IDC_TFD_ADMINMENU_G_MSGS: {
        LOG("Selected Logs");
        ctrlSetFocus (_display displayCtrl IDC_TFD_ADMINMENU_MSGS);
