...
if (!isMultiplayer) then {
    (_display displayCtrl IDC_TFD_ADMINMENU_DASH_CURRADMIN) ctrlSetText "none (singleplayer)";
};

// Mission Maker's Notes
private _ctrlMissionNotes = (_display displayCtrl IDC_TFD_ADMINMENU_G_DASH_MISSIONNOTES) controlsGroupCtrl IDC_TFD_ADMINMENU_DASH_MISSIONNOTES;
...
// Safestart
private _ctrlCheckSafestart = _display displayCtrl IDC_TFD_ADMINMENU_DASH_SAFESTART;
...
// Talk to spectators
private _ctrlCheckSpectatorTalk = _display displayCtrl IDC_TFD_ADMINMENU_DASH_SPECTATORTALK;
...
if (alive player) then {
    _ctrlCheckSpectatorTalk ctrlAddEventHandler ["CheckedChanged", {
        ...
        systemChat format ["[TFD Admin Menu] Spectator talk toggled %1", ["off", "on"] select _state];
...
private _ctrlReset = _display displayCtrl IDC_TFD_ADMINMENU_PMAN_FILTER_RESET;
_ctrlReset ctrlAddEventHandler ["ButtonClick", {
    params ["_ctrlButton"];
    _ctrlButton call FUNC(debounceButton);

    private _display = ctrlParent _ctrlButton;
    (_display displayCtrl IDC_TFD_ADMINMENU_PMAN_FILTER_SIDE) lbSetCurSel 0;
    (_display displayCtrl IDC_TFD_ADMINMENU_PMAN_FILTER_STATE) lbSetCurSel 0;
    _display call FUNC(playerManagement_updateList);
}];

private _ctrlRefresh = _display displayCtrl IDC_TFD_ADMINMENU_PMAN_REFRESH;
...
private _ctrlSelectAll = _display displayCtrl IDC_TFD_ADMINMENU_PMAN_SEL_ALL;
_ctrlSelectAll ctrlAddEventHandler ["ButtonClick", {
    params ["_ctrlButton"];
    _ctrlButton call FUNC(debounceButton);

    private _display = ctrlParent _ctrlButton;
    private _ctrlList = _display displayCtrl IDC_TFD_ADMINMENU_PMAN_LIST;
...
private _ctrlSelectNone = _display displayCtrl IDC_TFD_ADMINMENU_PMAN_SEL_NONE;
...
private _ctrlSelectInvert = _display displayCtrl IDC_TFD_ADMINMENU_PMAN_SEL_INVERT;
