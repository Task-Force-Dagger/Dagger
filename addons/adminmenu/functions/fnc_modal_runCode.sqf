...
_ctrlEdit ctrlSetPosition [0.1 * TFD_ADMINMENU_STD_WIDTH, 1.1 * TFD_ADMINMENU_STD_HEIGHT, _ctrlGrpWidth - (0.2 * TFD_ADMINMENU_STD_WIDTH), _ctrlGrpHeight - (2.3 * TFD_ADMINMENU_STD_HEIGHT)];
...
_ctrlHintEdit ctrlSetPosition [0, 0, _ctrlGrpWidth, TFD_ADMINMENU_STD_HEIGHT];
_ctrlHintEdit ctrlCommit 0;
_ctrlHintEdit ctrlSetText "'_this' is the targetted player object";

private _bottomY = _ctrlGrpHeight - TFD_ADMINMENU_STD_HEIGHT;

private _ctrlHintCombo = _display ctrlCreate [QGVAR(RscText), -1, _ctrlGroup];
_ctrlHintCombo ctrlSetPosition [0, _bottomY, 0.15 * _ctrlGrpWidth, TFD_ADMINMENU_STD_HEIGHT];
...
_ctrlCombo ctrlSetPosition [0.15 * _ctrlGrpWidth, _bottomY, 0.25 * _ctrlGrpWidth, TFD_ADMINMENU_STD_HEIGHT];
...
_ctrlButton ctrlSetPosition [0.8 * _ctrlGrpWidth, _bottomY, 0.2 * _ctrlGrpWidth, TFD_ADMINMENU_STD_HEIGHT];
...
_ctrlButton ctrlAddEventHandler ["buttonClick", {
    ...
    if (_editText isEqualTo "") then {
        systemChat "[TFD Admin Menu] Code field is empty";
    } else {
        ...
        systemChat format ["[TFD Admin Menu] Code was executed on %1", _ctrlCombo lbText (lbCurSel _ctrlCombo)];
