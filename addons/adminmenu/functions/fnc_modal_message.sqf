...
private _ctrlEditPos = [0.1 * TFD_ADMINMENU_STD_WIDTH, 0.1 * TFD_ADMINMENU_STD_HEIGHT, _ctrlGrpWidth - (0.2 * TFD_ADMINMENU_STD_WIDTH), _ctrlGrpHeight - (1.3 * TFD_ADMINMENU_STD_HEIGHT)];
...
_ctrlCombo ctrlSetPosition [0.1 * TFD_ADMINMENU_STD_WIDTH, _ctrlGrpHeight - TFD_ADMINMENU_STD_HEIGHT, _ctrlGrpWidth * 0.3, TFD_ADMINMENU_STD_HEIGHT];
...
_ctrlButtonPreview ctrlSetPosition [(_ctrlGrpWidth * 0.7) - (0.2 * TFD_ADMINMENU_STD_WIDTH), _ctrlGrpHeight - TFD_ADMINMENU_STD_HEIGHT, (_ctrlGrpWidth * 0.15), TFD_ADMINMENU_STD_HEIGHT];
...
_ctrlButtonPreview ctrlAddEventHandler ["buttonClick", {
    ...
    if (_editText isEqualTo "") then {
        systemChat "[TFD Admin Menu] Message can't be empty";
    } else {
        ...
        if (_venue isEqualTo QFUNC(showSubtitle)) then {
            ...
            if (_venue isEqualTo "hint") then {
                ...
                systemChat format ["[TFD Admin Message] %1", _editText];
            };
        };

        systemChat "[TFD Admin Menu] Message previewed";
    };
}];

private _ctrlButtonCommit = _display ctrlCreate [QGVAR(RscButtonMenu), -1, _ctrlGroup];
_ctrlButtonCommit ctrlSetPosition [(_ctrlGrpWidth * 0.85), _ctrlGrpHeight - TFD_ADMINMENU_STD_HEIGHT, (_ctrlGrpWidth * 0.15) - (0.1 * TFD_ADMINMENU_STD_WIDTH), TFD_ADMINMENU_STD_HEIGHT];
...
_ctrlButtonCommit ctrlAddEventHandler ["buttonClick", {
    ...
    if (_editText isEqualTo "") then {
        ...
        if (_venue isEqualTo QFUNC(showSubtitle)) then {
            ...
            (format ["[TFD Admin Message] %1", _editText]) remoteExec [_venue, GVAR(utilityData)];
        };

        systemChat "[TFD Admin Menu] Message sent to player(s)";
