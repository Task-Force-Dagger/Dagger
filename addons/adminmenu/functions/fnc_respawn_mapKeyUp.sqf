...
if (_type == 28) then {
    if (GVAR(respawnMousePos) isEqualTo -1) then {
        ...
        // Close the Map
        ((findDisplay IDC_TFD_ADMINMENU_RESP_MAP_DISPLAY) displayCtrl IDC_TFD_ADMINMENU_RESP_MAP_CONTROL) ctrlShow false;
        ((findDisplay IDC_TFD_ADMINMENU_RESP_MAP_DISPLAY) displayCtrl IDC_TFD_ADMINMENU_RESP_MAP_CONTROL) mapCenterOnCamera false;
        closeDialog IDC_TFD_ADMINMENU_RESP_MAP_DISPLAY;
    };
};
