...
/* ----------------------------------------------------------------------------
Function: TFD_adminmenu_fnc_log
...
Parameters:
    ...
    _tag - Tag that message will be prefixed with [String, defaults to "[TFD Log]"]
...
Examples:
    (begin example)
        ["Everything is on fire!",true,"[TFD Firealarm]"] call TFD_adminmenu_fnc_log;
    (end)
    (begin example)
        [format ["%1 did something",profileName]] call TFD_adminmenu_fnc_log;
    (end)
